...
(function () {
  ...
  async function searchKunde(q) {
    const res = await fetch(`${API_CUSTOMERS}?q=${encodeURIComponent(q)}`, { headers: { 'Accept': 'application/json' } });
    if (!res.ok) return [];
  ...
  function customerDisplay(c) {
    const kn = String(c.kunden_nummer ?? c.kunde_nummer ?? c.kundennummer ?? c.nr ?? c.address_no ?? '').trim();
    const name = String(c.company ?? c.firma ?? c.name ?? '').trim();
    if (kn && name) return `KN ${kn} – ${name}`;
    if (kn) return `KN ${kn}`;
  ...
  function phonesToString(c) {
    if (Array.isArray(c.phones)) {
      const out = c.phones.map((p) => {
        ...
        if (typeof p === 'object') {
          return String(p.number ?? p.phone ?? p.value ?? '').trim();
        }
        return '';
      }).filter(Boolean);
  ...
  function wireCustomerAutocomplete() {
    ...
    function hide() { box.hidden = true; box.innerHTML = ''; open = false; }

    function show(items) {
      if (!items || items.length === 0) { hide(); return; }
      box.innerHTML = items.map((it) => {
        const title = escHtml(String(it.company ?? it.firma ?? it.name ?? '').trim());
        const street = String(it.street ?? it.strasse ?? '').trim();
        const plz = String(it.postal_code ?? it.plz ?? '').trim();
        const ort = String(it.city ?? it.ort ?? '').trim();
        const addr = escHtml([street, [plz, ort].filter(Boolean).join(' ')].filter(Boolean).join(', '));
        const payload = encodeURIComponent(JSON.stringify(it));
        return `<button type="button" class="be_suggest_btn" data-json="${payload}">
                  <div class="be_suggest_title">${title}</div>
                  <div class="be_suggest_sub">${addr}</div>
                </button>`;
      }).join('');
      box.hidden = false;
      open = true;
    ...
    input.addEventListener('input', () => {
      ...
      clearTimeout(t);
      t = setTimeout(async () => {
        const items = await searchKunde(q);
        show(items);
      }, 200);
    });
    ...
    box.addEventListener('click', (ev) => {
      ...
      input.value = String(c.company ?? c.firma ?? c.name ?? '').trim();
      fillCustomer(c);
      hide();
    });
  ...
  function addEinsatzRow() {
    ...
    row.innerHTML = `
      <div class="be_e_cell be_e_date">
        ...
        <input class="input be_e_dat" name="e_dat[]" type="date" value="${escHtml(dateVal)}">
      </div>

      <div class="be_e_cell">
        <label class="label">Beginn</label>
        <input class="input be_e_start" name="e_start[]" type="time">
      </div>

      <div class="be_e_cell">
        <label class="label">Ende</label>
        <input class="input be_e_end" name="e_end[]" type="time">
      </div>

      <div class="be_e_cell be_e_durcell">
        <label class="label">Einsatzzeit (min)</label>
        <input class="input be_e_dur" name="e_dur[]" type="text" inputmode="numeric" placeholder="" readonly>
      </div>

      <div class="be_e_cell be_e_mitcell">
        <label class="label">Mitarbeiter</label>
        <input class="input be_e_mit" name="e_mit[]" type="text" value="${escHtml(def)}">
      </div>

      <div class="be_e_cell be_e_delcell">
        <label class="label">&nbsp;</label>
        <button class="btn be_row_del" type="button" title="Zeile löschen">×</button>
      </div>
    `;
  ...
  function addMaterialRow() {
    ...
    row.innerHTML = `
      <div class="be_m_cell be_m_art autocomplete">
        ...
        <div class="suggest m_suggest" hidden></div>
      </div>
  ...
  const sigState = {
    mainSigned: false,
    dasiSigned: false,
    touched: { main: { name: false }, dasi: { name: false } }
  };
  ...
  function getSignerNameUpperFor(kind) {
    const el = qs(kind === 'dasi' ? '#be_sig_dasi_name' : '#be_sig_main_name');
  ...
  function syncSignerFieldsFromDefaults(force = false) {
    ...
    if (mainName && (!sigState.touched.main.name || force)) {
      if (!String(mainName.value || '').trim() || force) mainName.value = defMain;
    }
    if (dasiName && (!sigState.touched.dasi.name || force)) {
      if (!String(dasiName.value || '').trim() || force) dasiName.value = defDasi;
    }
  }
  ...
  function drawSignatureWatermark(canvas, ctx, nameUpper, dateDE) {
    ...
    ctx.save();
    ctx.fillStyle = 'rgba(0,0,0,0.80)'; // WICHTIG: schwarz(er) für PDF
    ctx.font = '24px Arial';
